---
name: initialize
description: Start or rotate the active feature cycle under .cursor/features. Asks for a feature name, creates a dated folder YYYY-MM-DD_Name for artifacts, links .cursor/features/current to it, and archives any previous cycle.
command: /initialize
---

You are setting up the **canonical workspace for one feature cycle**. Every other skill in the chain (`/ideate`, `/plan`, `/implement`, `/review`, `/document`) resolves artifact paths relative to **`.cursor/features/current/`** at the **repository (workspace) root**.

**`current` may be a symlink** to a dated folder (e.g. `2026-03-27_MyFeature`). Tools and skills should follow it; artifact paths stay **`.cursor/features/current/`** for compatibility.

---

## Feature artifact root

**Active path (resolve via symlink):** `[workspace-root]/.cursor/features/current/`

**Physical folder (this cycle):** `[workspace-root]/.cursor/features/YYYY-MM-DD_SanitizedFeatureName/`
...
|---|----------|-------------|
| (none) | — | `/context-gathering` saves to `.cursor/context/` only — not part of the numbered set |
...
If `.cursor/features/DATE_SanitizedName` already exists, append a disambiguator: `_2`, `_3`, … until unused.
...
2. Compute **`DATE`**, sanitize to **`SanitizedName`**, resolve final **`TARGET_DIR`** = `.cursor/features/DATE_SanitizedName` (with disambiguation if needed).
...
2. Ensure `.cursor/features/` exists (create if missing).
3. If **`.cursor/features/current`** does **not** exist, skip to Stage 2.

4. If **`current` is a symbolic link** to a directory under `.cursor/features/`:
...
5. If **`current` is a plain directory** (legacy layout):
   ...
   - **Rename** (move) the directory:
     - **From:** `.cursor/features/current`
     - **To:** `.cursor/features/DATE_ArchiveSanitizedName`  
...
2. Add **`TARGET_DIR/README.md`** with:
   ...
   - Line 2: **Started:** `DATE`.
   - Line 3: Note that **`/initialize`** retires `current` and starts a new dated folder; numbered artifacts `1_`–`4_` and (on `/document`) `0_Overview.md` go in this folder via **`.cursor/features/current/`**.
3. From **`.cursor/features/`**, set **`current`** to point at the new folder:
   - **Preferred:** `current` → symbolic link to **`TARGET_DIR`** using a **relative** target (e.g. `ln -sfn 2026-03-27_MyFeature current` from inside `.cursor/features`).
...
> **Artifacts directory:** **`.cursor/features/<DATE>_<Name>/`**  
> **Active path (for skills):** **`.cursor/features/current/`** → same folder  
> Next: `/ideate`, `/plan`, etc. will save numbered artifacts there.

If a legacy folder was renamed in Stage 1, add one line:

> Previous cycle saved as **`.cursor/features/<DATE>_<ArchiveName>/`**
