...
1. **`/initialize`** — Create the dated feature folder and set `.cursor/features/current/` before any other skill runs.
...
8. **`/document`** — Collate artifacts into **`.cursor/features/[date_name]/`** and a readable **overview** plus index.
...
| Command | Skill | Role |
|--------|--------|------|
| `/initialize` | **initialize** | **Starts a feature cycle**: asks for a feature name, creates **`.cursor/features/YYYY-MM-DD_Name/`**, links `current` to it, and archives any previous `current`. Run this before `/ideate`. |
...
| `/ideate` | **ideate** | Structured **interview** (small batches of questions), then a full **PRD** and refinement pass; explicit **sign-off** before planning. Saves `1_ProductRequirementsDocument.md` under `.cursor/features/current/`. |
| `/plan` | **plan** | Reads the PRD (and context you’ve loaded); produces an **adaptive** phased plan (no boilerplate sections); refinement pass; **sign-off** before `/implement`. Saves `2_Plan.md` under `.cursor/features/current/`. |
| `/implement` | **implement** | Requires a signed-off plan; executes **phase → task** order; **tests with each task**; stops on real ambiguity; saves `3_Implementation.md` under `.cursor/features/current/`. No scope improvisation or drive-by refactors. |
...
| `/review` | **review** | **Second-agent, cold** review: PRD/plan/summary + code; checklist (alignment, correctness, patterns, edge cases, tests, security, performance, a11y, etc.); severity-ranked issues; saves `4_Review.md` under `.cursor/features/current/`. |
| `/document` | **document** | **Archivist**: gathers PRD, plan, impl summary, review; builds **`.cursor/features/YYYY-MM-DD_Name/`** with a standalone **`0_Overview.md`** and maintains a **`.cursor/features/README.md`** index. |
...
| **`.cursor/features/current/`** | Symlink to the active cycle's dated folder; all skills read/write here. |
| **`.cursor/features/YYYY-MM-DD_Name/`** | Numbered artifacts for one cycle: `1_ProductRequirementsDocument.md`, `2_Plan.md`, `3_Implementation.md`, `4_Review.md`, and (on `/document`) `0_Overview.md`. |
| **`.cursor/features/README.md`** | Index of all completed feature cycles; maintained by `/document`. |
